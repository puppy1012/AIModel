...
def parse(args=None):
    ...
    parser.add_argument('--dataset', type=str, help='dataset', default='wavData', choices=['wavData','voiceModel'])
    ...
    parser.add_argument('--repeat_G', type=int, default=1, help='how much repeat G')
    ...
    parser.add_argument('--l1', type=float, default=1.0, help='lambda 1')
    parser.add_argument('--l2', type=float, default=1.0, help='lambda 2')
    ...
    parser.add_argument('--sample_interval', type=int, default=1, help='interval of sampling images')
    parser.add_argument('--save_interval', type=int, default=1, help='interval of saving models')
...
n_iter_G = args.iterG #G 학습횟수
n_iter_D = args.iterD #D 학습횟수
...
inter_annos = np.zeros(
    (10*len(interpolating_attributes), len(selected_attributes)), dtype=np.float32
)
...
test_annos = np.zeros(
    (len(test_attributes), len(selected_attributes)), dtype=np.float32
)
...
test_dataset = list(os.listdir('C:/Users/ICT/Desktop/youda/AI_TeamRepo/testDatasetv10'))
test_dataloader = data.DataLoader(test_dataset, batch_size=batch_size)

from data import wavData, PairedData
train_dset = wavData(
'C:/Users/ICT/Desktop/youda/AI_TeamRepo/testDatasetv10', selected_attrs=selected_attributes, mode='train', val_num=1000
)
train_data = PairedData(train_dset, batch_size, mode='train')
print("train dataset size:",len(train_dset))
valid_dset = wavData(
'C:/Users/ICT/Desktop/youda/AI_TeamRepo/testDatasetv10', selected_attrs=selected_attributes, mode='val', val_num=1000
)
valid_data = PairedData(valid_dset,batch_size, mode='val')

print('# of Total Images:', len(train_data)+len(valid_data), '( Training:', len(train_data), '/ Validating:', len(valid_data), ')')
...
for ite in range(start_step, start_step+n_steps):
    ...
    for d in range(n_iter_D):
        print(f"{d} D trian")
        errD = gan.train_D(img_a, img_b, img_c, vec_ab, vec_ac, vec_bc)
    
    for g in range(n_iter_G):
        print(f"{g} G train")
        errG = gan.train_G(img_a, img_b, img_c, vec_ab, vec_ac, vec_bc)
    t_end = time.time()
    ...
    if (ite+1) % sample_interval == 0:
        gan.eval()
        with torch.no_grad():
            '''sqeeze() : 1인 차원 삭제, unsqeeze(): 1인 차원 생성'''
        ...
        for anno in inter_annos:
            ...
            f = torch.cat([f, img_inter.unsqeeze(1).cpu()], dim=1)
