# import randomsummary
import numpy as np
import torch
import torch.autograd as autograd
import torch.nn as nn
import torch.optim as optim
from RelGAN.switchable_norm import SwitchNorm2d
from RelGAN.torchsummary import summary
...
def tile_like(x, target):  # tile_size = 256 or 4
    print(x.shape , "target: ",target.shape)
...
class ResidualBlock(nn.Module):
    def __init__(self, n_in, n_out, kernel_size):
        ...
        self.f = nn.Sequential(
            nn.Conv2d(n_in, n_out, kernel_size=kernel_size, padding=0, stride=1), 
            SwitchNorm2d(n_out, momentum=0.9), 
            nn.ReLU(inplace=True), 
            nn.Conv2d(n_out, n_out, kernel_size=kernel_size, padding=0, stride=1), 
            SwitchNorm2d(n_out, momentum=0.9), 
        )
    def forward(self, x):
        return x + self.f(x)
...
class GAN(nn.Module):
    def __init__(self, args):
        ...
        self.G = G(1, self.latent_size, self.repeat_G)
        self.D = D(1, self.latent_size)
    ...
    def train_D(self, img_a, img_b, img_c, z_ab, z_ac, z_cb):
        #cuda에 자동으로 올라가지 않은 데이터들 수동으로 업로드
        img_a = img_a.cuda()
        img_b = img_b.cuda()
        img_c = img_c.cuda()
        ...
        # print("lambda3: ", self.lambda3.type, "di_loss: ", di_loss.type())
    ...
    def summary(self):
        ...
        g = summary(G, [(1, 256, 256), (self.latent_size, )], 
        ...
        d = summary(D, [(1, 256, 256), (1, 256, 256), (self.latent_size, )], 
                    dtype=[torch.float, torch.float, torch.float], 
                    use_gpu=self.gpu, return_str=True)
        # lambda *x: self.D(*x, critic=True)
        d_critic = summary(D, [(1, 256, 256)], 
                           dtype=[torch.float], 
    ...
    def load(self, file):
        ...
        if 'G' in states:
            self.G.load_state_dict(states['G'],strict=False)
        if 'D' in states:
            self.D.load_state_dict(states['D'],strict=False)
        # if 'optimG' in states:
...
class G(nn.Module):
    def __init__(self, n_c, n_z, n_repeat=6):
        ...
        resb_layers = [ResidualBlock(256, 256, 1) for _ in range(n_repeat)]
    ...
    def forward(self, img, z):
        print("image: ", img.shape, "z: ", z.shape)
        tiled_z = tile_like(z, img)
        print("tiled_z: ",tiled_z.shape)
        # img = img.float().type()
        # tiled_z = tiled_z.float().type()
        img = img.cuda()
        x = torch.cat([img, tiled_z], dim=1)
        x.float().type()
        # print("x shape: ",x.shape, "img: ",img.shape, "tiled_z: ",tiled_z.shape)
        x = x.cuda()
