...
#GAN에 필요한 parameters
def RelGAN(emotion, wav_file_list):
    '''emotion : 감정 레이블을 리스트로 받음
        wav_file_list : 음성 파일 리스트'''
    ...
    #========================변경할 데이터들 입력하기========================
    img = np.load(wav_file_list).tolist()
    ...
    make_voice = npytomp4(npy=testSet[0], path=os.path.join(args.path,f"test3_{i}.wav"))
    #return 값 : 리스트 안에 path 저장하기
    '''Mel Soectrogram화 한 사진 하나 보내주기'''

    #=============================변환한 음성을 리스트에 넣어 반환하기=============================
    conversion_voice = []
    conversion_voice.append(make_voice)
    return conversion_voice
